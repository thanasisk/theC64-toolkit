...
func main() {
	...
	fw.Seek(old_offset, 0)
	no_of_ext_sections := (88 - getFileOffset(fw)) / 8
	fmt.Println(no_of_ext_sections)
	// sections are grouped together and we have 4
...
func getEncryptionKey() []byte {
	/*
		seed, err := hex.DecodeString("650d9c3dc1860e6be17b3f55be1bf3825af718a43df1c510819ccf87e5ccf214")
		check(err)
		fmt.Println(seed)
			key[0] = 0x66C03166;
			key[1] = 0xF366DF89;
	...
	*/
	...
	check(err)
	//xor, err := hex.DecodeString("66C03166F366DF89E08966ABEFD3E866")
	xor, err := hex.DecodeString("EFD3E866E08966ABF366DF8966C03166")
	check(err)
	c := make([]byte, len(foo))
	for i := range foo {
		c[i] = foo[i] ^ xor[i]
